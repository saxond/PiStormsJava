...
public class PSSensor implements Constants {
	...
	/**
	 * Reads the specified amount of data into the {@link #data} array.
	 * length cannot be greater than 32.
	 */
	public void retrieve(int length) throws IOException {
		...
		/*
		int ready = i2cDevice.read(PS_EV_Ready + port.getMode());
		byte[] id = new byte[16];
		i2cDevice.read(PS_EV_SensorID + port.getMode(), id, 0, id.length);
		
		int mode = i2cDevice.read(PS_EV_Mode + port.getMode());
		int length = i2cDevice.read(PS_EV_Length + port.getMode());
		
		byte[] data = new byte[32];
		i2cDevice.read(PS_EV_Data + port.getMode(), data, 0, data.length);
				//PS_EV_Data + port.getMode(), data, 0, data.length);
